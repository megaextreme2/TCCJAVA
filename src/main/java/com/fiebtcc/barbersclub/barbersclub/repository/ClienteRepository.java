...
public interface ClienteRepository extends JpaRepository<Cliente,Long> {
    @Query(value = "SELECT * FROM Telefone cl WHERE cl.codStatus='1'", nativeQuery = true)
    public List<Cliente> listarTodosClientesAtivos();

    @Query(value = "SELECT * FROM Telefone cl WHERE cl.id=?1 AND cl.codStatus='1'", nativeQuery = true)
